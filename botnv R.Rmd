...
botnv <- unite(botnv, date, year, landing.month, landing.day, sep='-', remove=F)
botnv$date <- as.Date(botnv$date)
...
botnvWp <- botnv %>% group_by(year, week) %>% 
  summarize(cod = sum(cod),
            ...
            saithe = sum(saithe))
plot(botnvWp)
plotweek <- gather(botnWp, key = species, value = catch, cod, haddock, saithe) 
...
cor.test(botnvW$haddock, botnvW$saithe, method="pearson")
cor.test(botnvW$haddock, botnvW$cod, method="pearson")
cor.test(botnvW$saithe, botnvW$cod, method="pearson")
